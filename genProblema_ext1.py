...
npred = 0
nprep = int(raw_input('nombre maxim de preparadors: '))
nobjsdia = 0
for i in range(0,nex):
	...
	for j in range(0,nprepi):
		...
		'''while p == nom or (p in ex_rel.keys() and (nom in ex_rel[p][0] or nom in ex_rel[p][1])) or p in ex_rel[nom][1]:
			p = 'ex'+str(random.randint(0,nex-1))'''
		if p == nom or (p in ex_rel.keys() and (nom in ex_rel[p][0] or nom in ex_rel[p][1])) or p in ex_rel[nom][1]:
			nprepi -= 1
		else :
			...
			setOps.add(op+' ?x ?y)')
	if not teprep:
		for d in dies[0].split(' ')[1:16]: ex[1] += ' (te-preparador ' + nom + ' ' + d + ' num' + str(nprepi) + ')'
		tepred = True
	ex[1] += '\n\t\t'
	
ex[0] += '\n\t\t'
ex[1] += '\n\t\t'
for i in range(0,nprep+1):
	ex[0] += 'num' + str(i) + ' '
	if not i == nprep:
		op = '(decrP'
		ex[1] += ' ' +op + ' num' + str(i+1) + ' num' + str(i) + ') '
		setOps.add(op+' ?x ?y)')
